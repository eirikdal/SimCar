...
let argmax (list1 : float<'T> list) (list2 : float<'T> list) = list1 |> List.map2 (fun x y -> if x > y then x else y) list2
let sumn (list : float<'T> list list) : float<'T> list = list |> List.fold (fun ac dist -> sum2 ac dist) (List.init (96) (fun _ -> 0.0<_>))
let argmaxn (list : float<'T> list list) : float<'T> list = list |> List.fold (fun ac dist -> argmax ac dist) (List.init (96) (fun _ -> 0.0<_>))
let avgn lists = 
